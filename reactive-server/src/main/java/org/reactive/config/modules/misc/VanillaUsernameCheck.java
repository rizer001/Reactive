    ...
    public void onLoaded() {
        ...
        if (!regexString.isBlank()) {
            try {
                ...
                ReactiveConfig.LOGGER.error("Invalid username regex {} found, falling back to default.", regexString, e);
            }
        }
        if (useUsernameRegex && removeAllCheck) {
            ReactiveConfig.LOGGER.warn("Found conflicting configuration, remove-all-check and use-username-regex cannot be enabled at same time, ignoring remove-all-check...");
