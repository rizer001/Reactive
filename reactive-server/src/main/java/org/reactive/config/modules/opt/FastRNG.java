...
public class FastRNG extends ConfigModules {
    ...
    public void onLoaded() {
        ...
        if (enabled) {
            try {
                Class.forName("org.reactive.util.math.random.FasterRandomSource");
            } catch (Throwable ignored) {
                ReactiveConfig.LOGGER.error("Faster random generator is enabled but Xoroshiro128PlusPlus is not supported by your JVM, " +
        ...
        if (enabled && warnForSlimeChunk) {
            ReactiveConfig.LOGGER.warn("You enabled faster random generator, it will offset location of slime chunk");
            ReactiveConfig.LOGGER.warn("If your server has slime farms or facilities need vanilla slime chunk,");
            ReactiveConfig.LOGGER.warn("set performance.faster-random-generator.use-legacy-random-for-slime-chunk " +
                "to true to use LegacyRandomSource for slime chunk generation.");
            ReactiveConfig.LOGGER.warn("Set performance.faster-random-generator.warn-for-slime-chunk to false to " +
