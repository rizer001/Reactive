...
public abstract class ConfigModules extends ReactiveConfig {

    private static final Set<ConfigModules> MODULES = new HashSet<>();

    public ReactiveGlobalConfig config;

    public ConfigModules() {
        this.config = ReactiveConfig.config();
    }
    ...
    public static void initModules() throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        Class<?>[] classes = ReactiveConfig.getClasses(ReactiveConfig.I_CONFIG_PKG).toArray(new Class[0]);
        ...
        if (!enabledExperimentalModules.isEmpty()) {
            ReactiveConfig.LOGGER.warn("You have following experimental module(s) enabled: {}, please proceed with caution!", formatModules(enabledExperimentalModules));
        }

        if (!deprecatedModules.isEmpty()) {
            ReactiveConfig.LOGGER.warn("The following enabled module(s) has been deprecated: {}, please proceed with caution!", formatModules(deprecatedModules));
        }
    }
    ...
    public static void loadAfterBootstrap() {
        ...
        // Save config to disk
        try {
            ReactiveConfig.config().saveConfig();
        } catch (Exception e) {
            ReactiveConfig.LOGGER.error("Failed to save config file!", e);
        }
    }
